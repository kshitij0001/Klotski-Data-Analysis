# backend/scripts/enhanced_data_generator_heuristics_multicore.py
...
def _worker_generate_and_solve(args):
    """
    Worker function executed in subprocess. Returns a list of row-dicts (one per step if
    save_intermediate_steps, else single).
    args: tuple(sample_idx, save_intermediate_steps, is_json, rows, cols, solver_timeout, heuristic_name)
    """
    # Import inside worker to avoid pickling issues and to keep worker self-contained
    try:
        ...
        return {'error': f'ImportError in worker: {e}', 'sample_idx': args}

    sample_idx, save_intermediate_steps, is_json, rows, cols, solver_timeout, heuristic_name = args
    start_time = time()

    # Build heuristic callable locally (kept simple and pickle-safe by name indirection)
    def _build_heuristic(name):
        """
        Returns callable h(board)->int using board.get_goal_block() and board.winning_row/col.
        Options: none, manhattan, manhattan_plus_blocks.
        """
        def h_none(board):
            return 0

        def h_manhattan(board):
            g = board.get_goal_block()
            return abs(g.row_pos - board.winning_row) + abs(g.col_pos - board.winning_col)

        def h_manhattan_plus_blocks(board):
            g = board.get_goal_block()
            man = abs(g.row_pos - board.winning_row) + abs(g.col_pos - board.winning_col)
            try:
                blk = count_blocking_pieces(
                    g, board.blocks, target_row=board.winning_row, target_col=board.winning_col
                )
            except Exception:
                blk = 0
            return int(man + blk)
        ...
        mapping = {
            ...
            "manhattan_plus_blocks": h_manhattan_plus_blocks,
        }
        return mapping.get(name, h_manhattan)

    try:
        # Generate puzzle
        blocks, block_counts = generate_random_klotski_board()

        # Handle nested list/tuple if returned
        ...
        def solve_puzzle(blocks_data, heuristic_name_local):
            """Run solver in-thread inside worker process with heuristic"""
            try:
                heur = _build_heuristic(heuristic_name_local)
                solver = Solver(list(blocks_data), 3, 1, heuristic=heur)
                solver.solve()
        ...
        try:
            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(solve_puzzle, blocks, heuristic_name)
        ...
        if save_intermediate_steps and solution_path and len(solution_path) > 1:
            ...
            # For consistency add step_number field if save_intermediate_steps True (but no solution path to unfold)
...
def generate_enhanced_klotski_data(
    ...
    cols=4,
    solver_timeout=30,
    heuristic_name: str = "manhattan",  # NEW: plug-in heuristic
):
    """
    Generate comprehensive Klotski puzzle dataset with multicore processing and pluggable A* heuristics.

    Args:
        num_samples: Number of puzzles to generate
        output: Output file path (CSV or JSON)
        show_progress: Show progress bar (rich)
        show_details: Show detailed output for each puzzle (printed in main process)
        save_intermediate_steps: Save each step of solution path when available
        ...
        heuristic_name: 'none' | 'manhattan' | 'manhattan_plus_blocks'
    """
    if output is None:
    ...
    console.print(f"\n[bold green]Generating Enhanced Klotski Dataset (multicore)[/bold green]")
    console.print(f"Samples: {num_samples}")
    console.print(f"Output: {output}")
    console.print(f"Heuristic: {heuristic_name}")
    ...
    # CSV headers (kept same as prior versions for compatibility)
    csv_headers = [
    ...
    worker_args = [
        (sample_idx, save_intermediate_steps, is_json, rows, cols, solver_timeout, heuristic_name)
...
if __name__ == "__main__":
    console.print("[bold cyan]Enhanced Klotski Data Generator (multicore, A* heuristics)[/bold cyan]")
    ...
    try:
        heuristic_name = input("Heuristic (none/manhattan/manhattan_plus_blocks) [manhattan]: ").strip() or "manhattan"
    except Exception:
